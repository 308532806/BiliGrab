...
### 6.1 主色面：玻璃态下唯一跳过白雾的一类

第 1 步那层白雾对所有玻璃面都成立**除了主色面**。这条是 1.9.0 为修一个实机 bug 加上的。

**症状**：设置里选玻璃态，主题色的「深海蓝」标签按钮变成白色。

**根因**：`drawGlass()` 从头到尾只画 `glassTint`，<b>没有任何一处引用 `baseColor`</b> ——
也就是 `colors()` 设进去的底色在玻璃态下是个静默空操作。新拟态走实心填色路径，
所以看不出问题；一换玻璃态就露馅。而它只在"底色恰好需要一套对应字色"的那几套主题色上
明显（深海蓝是唯一白色字比黑色字对比度更高的，所以最先被注意到）。

**修法**是给主色面单开一条路径，而不是让色卡去将就白雾 —— 色卡存在的意义就是准确显示颜色：

| 方法 | 作用 |
| --- | --- |
| `glassAccent(int)` | 设主色。非 0 时走主色路径 |
| `accent(color, light, dark)` | 分派器：玻璃态走 `glassAccent`，新拟态走 `colors`。**不动** `style`/`emboss` |
| `faceColor()` | 返回「这个面实际呈现的颜色」，供在面上摆文字时算对比度 |

`drawGlass()` 里主色面先平铺 `glassAccent`，然后把白雾那一步用 `if (!accent)` 跳过；
方向光和渐变描边照旧 —— 所以主色面仍然有玻璃的明暗，只是没有那层把颜色冲淡的白。

### 6.2 `faceColor()`：四条渲染路径，四个结果

要在面上摆文字，就必须知道它到底渲染成什么颜色。不能拿设进去的颜色算：

| 路径 | 呈现颜色 |
| --- | --- |
| 新拟态 · 凸 | 就是 `baseColor`（只有边缘一圈高光，中间不受影响） |
| 新拟态 · 凹 | 内阴影是**描边式**的（宽 `2e` 再按 `e` 模糊），只压暗边缘十几像素 —— 文字坐在中间，所以也返回 `baseColor` |
| 玻璃 · 主色面 | `glassAccent` 本身（跳过白雾），凹面再乘 `0.84` |
| 玻璃 · 普通面 | `withAlphaOver(baseColor, glassTint)`，凹面再乘 `0.84` |

系数 `0.84` 是从实机截图**逐通道反推**的，不是估的 —— 玻璃态凹面的方向光从顶部
`0x42000000` 一路铺到 `0.78h`，整个面都被压暗，中间也不例外：
...
取 0.84 是让**字色的选择**在五套主题色上全部落在实测那一侧。这里踩过一次坑，值得记下：

> 一开始只在新拟态分支里做压暗，因为"内阴影"这个词让人以为两边都有。但玻璃态的凹陷
> 是靠**铺满全高的方向光**，新拟态才是描边式 —— 两者物理上不同。漏掉玻璃分支的后果是
> 默认蓝按本色算出黑字，落在被压暗的 `#0068D9` 上只有 **3.58:1**（白字有 5.88:1）；
> 薰衣草紫同理（黑 3.81 / 白 5.19）。实机上那两张卡的标签确实是灰的。
>
> 还试过"取整个亮度范围、选最差情形更好的那个"来兜住逐色浮动。那个范围要一直放宽到
> 0.62 才包得住全部观测，而这么宽的范围里**黑白两个候选都不达标**（最差 3.11:1），
> 比取单值更糟。所以这里用实测中值，不用区间。
...
| `hyper_text_primary` | `#111111` | `#F5F5F5` | 主文字（17.15:1 / 15.80:1） |
| `hyper_text_secondary` | `#505360` | `#B3B3B3` | 次要文字（6.94:1 / 8.21:1） |
| `hyper_text_tertiary` | `#6A6D7B` | `#8E8E93` | 三级文字 / hint（4.67:1 / 5.28:1） |
...
| `hyper_scroll_edge` | `#40000000` | `#40FFFFFF` | 滚动条滑块 + 滚到尽头的边缘光晕 |
...
> **1.9.0 调整了三个文字令牌**（次要/三级，浅深各一处）。原来那两级落在 4.5:1 上下 ——
> 属于"看着能读、但不过线"。括号里是对各自页面底色的实测比值。
...
#### 7.5.1 主色上的文字：三种场景，三个函数

主色是运行时可换的，所以"该配黑字还是白字"必须在运行时算。这里**不能只用一个函数** ——
背景不同，要算的目标色也不同：

| 函数 | 用在哪 | 算法 | 阈值 |
| --- | --- | --- | --- |
| `contrastOn(bg)` | 通用 | WCAG sRGB 去 gamma 后的相对亮度，黑白二选一 | 取对比度更高者 |
| `primaryText(ctx)` | 主色面上的**文字** | 先把主色按 `readableOn` 向页面底色混合，直到达标 | **4.5:1** |
| `primaryIcon(ctx)` | 主色面上的**图标** | 同上 | **3:1** |

分两档是因为图标按规范只需 3:1（非文字内容的门槛），而正文要 4.5:1。挤在一个函数里，
要么图标被过度处理、要么文字不达标。

`readableOn(color, bg, floor)` 在浅色页面上对樱花粉的实测效果：`#FB7299` 原本只有
**2.39:1**，逐档混到 `#AE526C` 才够 4.50:1。默认蓝 `#007AFF` 3.65 → `#026BDE` 4.58；
薄荷绿 2.24 → 4.55；薰衣草紫 3.03 → 4.54。深海蓝本来就够（6.80），不动。

> **删掉的东西**：旧版有个 `primaryVariant()`，取数组里的 variant 色（`#E04B78` 这类）
> 当"主色的浅色版"用。它在 1.9.0 被删了 —— 那是**设计者预先调好的一档色**，不是按
> 对比度算出来的，拿它当"能读的版本"用等于假设每套主题色的 variant 都刚好够，
> 而这个假设从没被验证过。改成按目标对比度现算，就不依赖这层假设。

#### 7.5.2 一个曾经漏掉整页的坑：`android:tag`

主色面只要在布局里写 `android:tag="neu:accent"`，`NeumAttr.applyAccentTags(root)` 就会
遍历整棵树把它刷成当前主色。用 tag 而不是自定义属性，是因为 `ProgressBar` 这类系统控件
由框架自己构造，读不到 `app:` 命名空间的属性。

**但每个 Activity 都必须自己调用一次 `applyAccentTags`。** 漏掉的话 tag 就是死的：
编译通过、运行不报错，控件安静地停在 `@color/scheme_0_primary`（樱花粉，第一套主题色的
编译期快照），只有换到非默认主题色才看得出来。

1.9.0 就踩了这个：`activity_downloads.xml` 的空状态图标标了 tag，但 `DownloadsActivity`
从没调过 `applyAccentTags` —— 实机上是**粉色的下载图标配着一整套蓝色界面**。
这类缺陷靠代码审查很难发现（tag 在那里、函数也在那里，只是没连上），只能靠实机换主题色截图。
