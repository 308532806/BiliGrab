...
**v1.7.0 封出来的 MP4，音轨可能是空的** —— 文件能播、有画面、时长也对，
但完全没有声音；仅音频则可能只写到 85% 就断，产物连样本表都没有、
根本打不开，却被标成「已完成」。根因是分片 MP4 边界上的重复起播帧
把 `OplusMPEG4Writer` 逼到判死整条轨，一个 34 分钟的文件里有 700 个。

### 现场数据

同一条 34 分钟的稿件（`BV1xx411c7mD` P1），同一台手机：

| | v1.7.0 | v1.7.1 |
|---|---|---|
| 仅音频大小 | 29,491,948 | **34,610,750** |
| 仅音频时长 | 29:43 | **34:15** |
...
`Probe2` 打出的时间戳现场（这是**关键证据**，别再重复查一遍）：

```
样本 77512  pts=1783529274
样本 77513  pts=1783529297   ← 只隔 23 微秒，正常是 23220
样本 77514  pts=1783552222   ← 又回到正常间距
```

源文件本身是干净的：412 个分片逐个核对 `tfdt`，**零处回退**，
`MediaExtractor` 遍历 89,223 个样本也**零处回退**。所以这不是「源坏了」，
是「边界上多了一个挤在同一时刻的帧，而封装器不接受」。

### 修法（三个改动）

1. **合成视频时丢掉贴在前一帧身上的重复帧**（`MuxUtil.copyTrack`）。
   判据：`step < typicalStep / 2`，`typicalStep` 从最初几个样本学出来，
   不依赖音频参数一定能取到。丢掉时**不动 `lastPts`**，后面的样本
   时间戳自己就接得上。

   **只对 AAC 生效**（`isAac()`）：Opus / Vorbis 帧长本来就变，
   套用会把好帧当异常扔掉。实测丢弃 700 个，写 88,523 个。

2. **仅音频不再重新封装**（`DownloadService.muxAndSave`）。
   源流本身就是完整的、能直接播的 m4a，拆开重写没有收益、只多一次
   会失败的操作。这条路径现在 `copyFile()` 流式复制。

   用流式复制而不是 `renameTo`：工作目录在应用私有区、目标可能在
   SAF 那边，跨文件系统的 rename 在 Java 层直接返回 false，
   失败时不抛异常、只给个布尔值。

3. **复制也要能被取消**。复制几十 MB 也是要时间的，第一版漏了查停止
   标志：用户点了取消、界面回到待下载，文件却还在写。

### 顺带修掉的两个校验缺陷

- **完整性校验以前只在 `stop()` 抛异常时才跑**。但坏文件恰恰是在
  `stop()` **正常返回**时产生的 —— 那条无声 MP4 就是这么出去的。
  现在无论 `stop()` 成不成功都验证。
  判据也从「有没有 moov」升级成「**样本数是不是 0**」：轨在、
  `mdat` 有 29 MB、样本表却是空的，这种文件以前能过检。

- **校验对分片 MP4 的误判**。它原本只认 moov 里的样本表，而分片文件的
  样本在 `moof` 里、moov 里本来就是空的，会把正确文件判成坏的。
  新增 `fragments` 计数与 `hasPlayableAudio()`。

### 这一轮踩到的工具坑（都是真踩过，不是预防性写的）

- **`push-via-api.py` 不校验第一个参数。** 参数顺序是
  `<tag> [说明] [基线]`，把基线提交当第一个参数传进来，它会在远端
  建一个名叫提交哈希的 tag（v1.7.0 那轮建出过 `refs/tags/4a9bce9`），
  而且照常输出「推送完成」，只有去列 tag 才会发现。现在直接拒绝。

- **`release-apk.py` 在 tag 被移动后会重复建 Release。**
  `move-tag.py` 删掉重建 tag，旧 Release 就与 tag 脱钩，按 tag 查是
  404，脚本当成「还没发过」又建一个 —— 远端于是有两个同 tag_name 的
  Release，点进去看到哪个全看运气，而旧的里面是修复前的 APK。
  实测踩到（`394662769` 与 `394694282`），已删旧的并修好工具：
  404 之后按 `tag_name` 在列表里捞，捞到就改它。

- **给已发布 Release 改说明必须走 Python。** PowerShell 5.1 按本地
  代码页编码请求体，中文被 GitHub 收成一串问号（实测踩到，
  页面顶了一段 `????`）。`tools/mark-superseded.py` 与
  `tools/fix-release-body.py` 就是为此写的；后者还修过一个更严重的：
  清理残骸时沿分隔线截断，**把 v1.7.0 原本的发布说明一起删了**，
  现在从 `docs/release-notes-1.7.0.md` 重建成「警示 + 原始说明」。

- **PowerShell 5.1 的 `.ps1` 必须有 UTF-8 BOM**，否则它按 GBK 读，
  中文注释会碎成乱码并把后面的语法带崩（这一轮 `cancel-test.ps1`
  因为这个连挂两次）。

- **PowerShell 字符串里不能出现 shell 的 `||`**，解析器会当成语句
  分隔符直接报错。

- **Android 不支持 `tc` 限速**（`qdisc mq` / `pfifo_fast` 都不支持），
  所以「限速到下载窗口够长再点取消」这条路在这台机器上走不通。
  改用「点完下载立刻进管理页」减少 adb 往返。

### 怎么验的

- 仅音频：落盘 34,610,750 字节，`Probe` 读出来 1 条轨、**89,223 个样本**、
  时长 **2055 秒 = 34:15**，与源文件逐个数字一致。
- 视频：`deepmp4.py`（自写 box 解析器）核对 `stsz` / `co64` / `stts` /
  `stsc` 四张表齐全，轨 0 `vide` 30,829 样本、轨 1 `soun` 88,523 样本，
  两条轨时长都是 34:15。
- 目录：`.m4a` 与 `.mp4` 都落在自定义目录 `MyGrab/`，记录里
  `location` 对得上。
- 管理页：两条记录都显示「已完成」与正确体积，`work` 目录干净。

### 还没验的（下一轮）

- **取消按钮在「下载正在跑」时的那条路径**，这一轮没能测到：
  下载 70 MB 只要十几秒，而 `tc` 限速这台机器不支持，轮询抓到
  RUNNING 之前任务就已经完成了。代码逻辑（`pendingCancelOnly`、
  `finishPendingDelete`）在上一轮验过，这一轮只多改了复制路径的
  停止检查，但那处**没有被实测覆盖**。
- 大会员 4K 路径始终没拿真实 cookie 走过。

### 发版产物

- Release `v1.7.1` id `394694282`，asset `583836881`，
  **19,472,170** 字节，SHA256 `A2CC95176E6EAF9444376E1E9B3FECE2A9286BDF5F078820C34025B2E66EE137`
  （下载回来核对过，与本地一致）。
- `v1.7.0` 的说明顶部加了警示，指向 v1.7.1。
- 远端 tag 只有 `v1.0.0` … `v1.7.1`。
